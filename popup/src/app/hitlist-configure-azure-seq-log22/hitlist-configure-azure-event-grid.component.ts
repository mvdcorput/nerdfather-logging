...
export class HitlistConfigureAzureEventGrid implements OnInit {
  ...
  ngOnInit() {
    this.target$ = combineLatest(
      [
        this.hitlistService.targettedUrl$,
      ...
      map(([currentTarget, url, domain, hitlist, queryParams]) => {
        const target = currentTarget ?
          currentTarget :
          hitlist.filter(t => this.appService.hiyaCode(t.url) === parseInt(queryParams['id'], 0))[0];
